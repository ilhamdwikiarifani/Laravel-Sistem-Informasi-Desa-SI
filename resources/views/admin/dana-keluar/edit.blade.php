...
@section('isi')
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-4 pb-4">
        <div class="d-block mb-4 mb-md-0">
            ...
            <h2 class="h4">Ubah Dana Keluar : {{ $danaKeluar->dana->nama }}</h2>
    ...
    <div class="col-xxl">
        <div class="card mb-4">
            ...
            <div class="card-body">
                <form method="POST" action="{{ route('dana_keluars.update', $danaKeluar->id) }}"
                                ...
                                <select name="" id="nikk"
                                    ...
                                    @foreach ($danas as $dana)
                                        <option value="{{ $dana->id }}"
                                            {{ old('dana_id', $danaKeluar->dana_id) == $dana->id ? 'selected' : '' }}>
                                            {{ $dana->nama }}</option>
                                    @endforeach
                                </select>
                                <input type="hidden" name="dana_id" value="{{ $danaKeluar->dana_id }}">
                                @error('dana_id')
                        ...
                        <div class="col-sm-10">
                            <div class="input-group input-group-merge">
                                ...
                                <textarea name="keterangan" id="basic-icon-default-message"
                                    class="form-control @error('keterangan') is-invalid @enderror" placeholder="">{{ old('keterangan', $danaKeluar->keterangan) }}</textarea>
                                @error('keterangan')
                                    <div class="invalid-feedback">
                                        {{ $message }}
                                    </div>
